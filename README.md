...
本项目把摄像头生命周期、检测参数和各类视觉算法分开。检测模块统一接收一帧 RGB 图像，不在导入时初始化摄像头、显示器或串口，因此可以在 `detectors/digit.py`、`detectors/rectangle.py` 或新的主程序中组合使用。
...
| `main.py` | 根目录自动运行入口；默认调用 `detectors.rectangle.run_rectangle_tracking()` |
| `core/camera_io.py` | `Sensor`、`Display`、`MediaManager` 生命周期 |
| `tools/wifi_rtsp.py` | 可选 Wi-Fi STA 与安全 H.264 RTSP 生命周期；默认关闭，失败回退原视觉流程 |
| `tools/safe_wbc_rtsp.py` | 项目内有限超时的 WBC/VENC/RTSP 实现，避免固件示例的无限等待 |
| `local_rtsp_viewer/` | 仅本机使用的网页查看器；全局只建立一个 FFmpeg/RTSP 上游 |
| `core/draw_utils.py` | OpenCV 图像和 AI Pipeline OSD 图层的统一文字绘制辅助函数 |
| `core/uart_io.py` | FPIOA、UART 生命周期、二进制数据帧、双向握手和目标偏差协议；可直接运行固定信号测试 |
| `core/bluetooth_uart.py` | UART2 蓝牙单字符指令接收与缓存，导出 `BluetoothUART` |
| `detectors/color_spot.py` | 彩色光点检测，导出 `ColorSpotDetector` |
| `detectors/road_symbol.py` | T/十字主连通轮廓和黑色双排虚线结束符检测，导出 `RoadSymbolDetector` |
| `detectors/line_track.py` | 红色引导线分带循迹和路口提示，导出 `LineTrackDetector`；直接运行时也是完整循迹程序 |
| `detectors/rectangle.py` | 黑框白心方框检测，导出 `RectangleDetector`；直接运行时也是完整追踪程序 |
| `detectors/pencil_rectangle.py` | 细铅笔线方框检测，导出 `PencilRectangleDetector`；多重方框中选择估算边框最细者 |
| `detectors/rectangle_corner_cycle.py` | 独立的方框四角顺时针停留、移动和串口输出应用 |
| `detectors/rectangle_distance.py` | 复用 `RectangleDetector` 检测方框，并用 cv2/PnP 估算距离 |
| `detectors/steelball.py` | KPU/YOLO11 钢球检测演示，绘制检测框、FPS 和数量 |
| `detectors/steelball_distance.py` | 复用钢球 YOLO11 检测框，根据 1cm 直径估算钢球距离 |
| `detectors/digit.py` | 打印数字检测，导出 `DigitDetector`；直接运行时也是完整识别程序 |
| `tools/capture.py` | 按需拍照并保存到 TF 卡，导出 `CaptureService`；直接运行时等待 CAPTURE 帧并存图 |
| `assets/` | 数字模板、KPU/kmodel 等资源文件 |

原 `rectangle_detector.py` 已合并进 `detectors/rectangle.py`，不再需要上传。

## 历史入口兼容表

旧文件仍可直接运行或导入，但只作为兼容转发入口；新代码请使用规范模块名。

| 旧入口 | 新模块 |
| --- | --- |
| `detectors/tangle.py` | `detectors/rectangle.py` |
| `detectors/num.py` | `detectors/digit.py` |
| `detectors/color.py` | `detectors/color_spot.py` |
| `detectors/line.py` | `detectors/line_track.py` |
| `detectors/road.py` | `detectors/road_symbol.py` |
| `detectors/square_distance.py` | `detectors/rectangle_distance.py` |
| `detectors/corner_cycle.py` | `detectors/rectangle_corner_cycle.py` |
| `detectors/steelball_detect.py` | `detectors/steelball.py` |
...
```python
from detectors.color_spot import ColorSpotDetector
from detectors.road_symbol import RoadSymbolDetector
from detectors.line_track import LineTrackDetector
from detectors.rectangle import RectangleDetector
from detectors.pencil_rectangle import PencilRectangleDetector
from detectors.rectangle_distance import RectangleDistanceDetector
from detectors.steelball_distance import estimate_steelball_distance
from detectors.digit import DigitDetector
...
rectangle_distance_detector = RectangleDistanceDetector()
...
rectangle_distance = rectangle_distance_detector.process(frame)
...
板载屏幕和 IDE 的分辨率、位置、帧率、质量参数分别由 `config.py` 中的 `BOARD_DISPLAY_...` 和 `IDE_DISPLAY_...` 管理。它们是所有摄像头程序共享的显示配置，不再与 `detectors/rectangle.py` 或 `detectors/digit.py` 绑定；`detectors/steelball.py` 也读取同一个 `DISPLAY_TARGET`。需要临时覆盖时仍然可以写 `CameraIO(display_target=DISPLAY_TARGET_BOARD)` 或 `CameraIO(display_target=DISPLAY_TARGET_IDE)`。当前摄像头同时启用水平镜像和垂直翻转，等效于旋转 180°。

## Wi-Fi RTSP 标注画面推流

`tools/wifi_rtsp.py` 使用 CanMV 固件内置的 `network.WLAN`、Display writeback 和 K230 硬件 H.264 VENC，把 `CameraIO.show_image()` 的最终显示画面推到局域网。检测框、文字、状态和 FPS 已经画在 Display 上，因此网页画面与原 IDE 画面内容一致并包含全部批注。
...
项目内 `tools/safe_wbc_rtsp.py` 保持官方协议参数：
...
`wifi_secrets.py` 已加入 `.gitignore`，不要提交真实密码。把它和 `tools/safe_wbc_rtsp.py`、`tools/wifi_rtsp.py`、`core/camera_io.py`、`config.py` 一起上传到 `/sdcard/K230`。
在 `config.py` 中开启：
...
`tools/safe_wbc_rtsp.py` 对 `SendFrame`、`GetStream` 和 RTSP 发送都使用有限超时，并把停止等待限制为 2 秒。如果工作线程仍未退出，它不会继续销毁 Display/MediaManager，而会报告必须断电重启，避免再次运行时访问已经释放的媒体对象。
...
`detectors/road_symbol.py` 只识别 T 型、十字型和黑色双排虚线结束符，不再返回普通 `line`。场地标记只有红色和黑色，因此 T/十字不再执行 HSV 红色识别：检测器对 RGB 图像的绿色通道执行 Otsu 反向二值化，把红色和黑色统一作为前景，再选择真正横跨左右、经过中心附近并向下延伸的单个主轮廓。与路线不连通的黑色数字、边框和远处物体不会参与 T/十字几何计算。
...
可直接运行 `detectors/road_symbol.py` 调用 `run_road_demo()`。演示默认读取 `config.py` 中的 `DISPLAY_TARGET`，并在显示前绘制当前主循环 FPS。

当前实拍目录回归结果：T 全部文件 `50/50`、按内容去重 `17/17`；十字全部文件 `14/14`、去重 `11/11`；结束符全部文件 `5/5`、去重 `4/4`。全部回归均在默认 `320×240` 内部检测分辨率下完成。

## 红线巡线模块

`detectors/line_track.py` 面向 2021 年电赛 F 题的红色引导线。它把画面下部切成 5 条水平带，每条带
内求出红线的横向位置，等效于一列沿前进方向排开的虚拟灰度传感器：最近一条带给
单片机做 PID，最远一条带提供弯道和路口的前瞻。

它与 `detectors/road_symbol.py` 分工不同，不能互相替代：`detectors/road_symbol.py` 用绿色通道 Otsu 把红色和黑色
...
```python
import detectors.line_track as line_track
...
数字识别的分支。该分支目前是 `TODO`，只在画面上显示 `JUNCTION`，接入 `detectors/digit.py` 的
...
`RectangleDetector` 每帧独立检测，不使用历史位置、ROI 或运动预测。`detectors/rectangle.py` 直接运行时的 `TargetHoldState` 只服务于示例画面显示，不属于检测器，也不会把历史坐标作为有效串口数据发送。

## 方框 PnP 距离检测模块

`RectangleDistanceDetector` 复用 `detectors.rectangle.RectangleDetector` 获取方框四角，然后估算相机到方框中心的距离。测距优先调用 `cv2.solvePnP`；如果当前 CanMV 固件没有暴露该函数，则使用模块内置的平面矩形 homography 后备算法。

```python
from detectors.rectangle_distance import RectangleDistanceDetector

distance_detector = RectangleDistanceDetector()
...
直接运行 `detectors/rectangle_distance.py` 会启动摄像头演示，并绘制：
...
| 参数 | 含义 |
| --- | --- |
| `RECTANGLE_DISTANCE_ROI` | 只接受中心点落入该区域的方框，格式为 `(x0, y0, x1, y1)` 画面比例 |
| `RECTANGLE_DISTANCE_OBJECT_WIDTH_CM` | 实物方框宽度，单位 cm |
| `RECTANGLE_DISTANCE_OBJECT_HEIGHT_CM` | 实物方框高度，单位 cm |
| `RECTANGLE_DISTANCE_CAMERA_MATRIX` | 相机内参矩阵，当前使用 01Studio K230 官方 PnP 示例参数 |
| `RECTANGLE_DISTANCE_DIST_COEFFS` | 畸变系数 |
| `RECTANGLE_DISTANCE_MIN_DISTANCE_CM` | 小于等于该值时视为未检测到有效距离 |

如果返回 `None`，通常是 `RectangleDetector` 没有找到有效方框、方框中心不在 ROI 内，或 PnP 计算失败。画面和终端会输出异常信息，按该信息调整方框检测参数、目标尺寸、ROI 或相机参数。

## 钢球距离判断模块

`detectors/steelball_distance.py` 复用 `detectors/steelball.py` 的 YOLO11 模型加载、显示配置、摄像头配置和检测结果解析。距离估算不使用 PnP，而是使用已知钢球直径：
...
`detectors/steelball.py` 的数量统计也改为先把 YOLO 返回值标准化为真实检测框列表，再统计长度；不再把未知容器长度兜底当作数量，避免左上角固定显示 `Num: 2` 或 `Num: 3`。
...
直接运行主程序时，会复用 `detectors/rectangle_corner_cycle.py` 的 `CornerCycleController` 和绘制函数，在所选细框的四个角之间执行 `TL -> TR -> BR -> BL -> TL` 轨迹，并通过 `TrackingUART` 发送当前插值点相对画面中心的偏差：
...
`detectors/rectangle_corner_cycle.py` 是独立应用，不把轨迹状态加入 `RectangleDetector` 或 `TrackingUART`。它只调用现有方框检测和串口接口，内部完成四角排序、计时和线性插值。
...
`tools/capture.py` 导出 `CaptureService`，收到 MSPM0 发来的 CAPTURE 帧（`TYPE=0x20`）后，在主循环的下一帧把当前画面保存到 TF 卡，然后回一个 `CAPTURE_ACK` 帧（`TYPE=0x21`）。照片不通过串口回传，事后拔卡拷贝。
...
```python
from tools.capture import CaptureService
...
`core/uart_io.py` 导出 `TrackingUART`。默认配置是：
...
`wait_for_handshake()` 只在程序启动阶段使用，内部调用 `update_handshake()`，周期发送 READY 并处理 READY/READY_ACK。只有既收到对方 READY、又收到对方对本机 READY 的 ACK，`handshake_complete` 才为真。双方都必须主动发送 READY，不能采用“先等待接收、收到后才发送”的流程，否则会互相死锁。单 K230 测试时，如果没有连接对端单片机，按下 GPIO21 按钮（低电平有效）可跳过握手等待并直接进入主程序；相关参数在 `config.py` 的 `UART_HANDSHAKE_SKIP_BUTTON_...` 中配置。
...
直接在 K230 上运行 `core/uart_io.py` 会执行通信测试：握手成功前保持等待；如果按下 GPIO21 按钮则跳过等待。握手成功或跳过后，每 10 ms 尝试发送固定 `valid=1、x=123、y=-45` TARGET 帧。天猛星 OLED 最后一行应显示：
...
不要在每一帧中重新创建或初始化 `TrackingUART`。导入 `core/uart_io.py` 不会占用 GPIO；调用 `initialize()` 时会初始化 UART 引脚，调用 `wait_for_handshake()` 时会按需配置 GPIO21 跳过按钮。

## 蓝牙串口模块

`core/bluetooth_uart.py` 导出 `BluetoothUART`，通过 UART2 接收蓝牙透明串口传来的单字符指令。当前只接受 `1`、`2`、`3`、`4`、`s`、`p`，自动忽略空格、制表符、回车和换行；其他字符会被报告并丢弃。该模块不通过蓝牙发送数据。
...
- `LINE_...`：红线分带循迹和路口判定。
- `RECTANGLE_...`：方框。
- `RECTANGLE_TRACK_...`：方框追踪演示的打印、回收和绘制参数；`TANGLE_...` 为历史别名。
- `RECTANGLE_CORNER_CYCLE_...`：四角轨迹的停留、移动、串口周期和绘制参数；`CORNER_CYCLE_...` 为历史别名。
...
- `detectors/color_spot.py`
- `config.py`

直接运行彩色光点摄像头示例还需要 `core/camera_io.py`。

寻路符号功能至少需要：

- `detectors/road_symbol.py`
- `config.py`

直接运行 `detectors/road_symbol.py` 的摄像头演示还需要 `core/camera_io.py`。

红线巡线功能至少需要：

- `detectors/line_track.py`
- `config.py`

直接运行 `detectors/line_track.py` 的循迹主程序还需要 `core/camera_io.py` 和 `core/uart_io.py`；
传入 `enable_uart=False` 时不需要 `core/uart_io.py`。

方框功能至少需要：

- `detectors/rectangle.py`
- `detectors/color_spot.py`（仅直接运行 `detectors/rectangle.py` 的组合示例需要）
- `config.py`
- `core/camera_io.py`（仅直接运行完整摄像头示例需要）
- `core/uart_io.py`（直接运行 `detectors/rectangle.py` 的串口追踪示例需要）

蓝牙串口功能至少需要：

- `core/bluetooth_uart.py`
- `core/uart_io.py`
- `config.py`

四角循环应用至少需要：

- `detectors/rectangle_corner_cycle.py`
- `detectors/rectangle.py`
- `core/uart_io.py`
- `core/camera_io.py`
- `config.py`
...
- `detectors/pencil_rectangle.py`
- `config.py`

直接运行 `detectors/pencil_rectangle.py` 的四角轨迹主程序还需要：

- `detectors/rectangle_corner_cycle.py`
- `detectors/rectangle.py`（`detectors/rectangle_corner_cycle.py` 当前使用其通用四边形绘制函数）
- `core/uart_io.py`
- `core/camera_io.py`
...
直接运行 `tools/capture.py` 的存图主程序还需要 `core/camera_io.py` 和 `core/uart_io.py`；
传入 `enable_uart=False` 时不需要 `core/uart_io.py`。
