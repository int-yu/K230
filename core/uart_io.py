...
from config import (
    ...
    UART_HANDSHAKE_SKIP_BUTTON_ACTIVE_VALUE,
    UART_HANDSHAKE_SKIP_BUTTON_DEBOUNCE_MS,
    UART_HANDSHAKE_SKIP_BUTTON_ENABLE,
    UART_HANDSHAKE_SKIP_BUTTON_PIN,
    UART_ID,
    UART_RX_PIN,
    UART_SEND_PERIOD_MS,
    UART_TX_PIN,
...
def _make_skip_button(machine_module=None):
    """创建握手跳过按钮；失败时返回 None，不影响正常串口握手。"""

    if not UART_HANDSHAKE_SKIP_BUTTON_ENABLE:
        return None
    ...
    try:
        ...
        pin_class = machine_module.Pin
        fpioa = fpioa_class()
        gpio_function = getattr(
            fpioa_class,
            "GPIO{}".format(UART_HANDSHAKE_SKIP_BUTTON_PIN),
        )
        fpioa.set_function(UART_HANDSHAKE_SKIP_BUTTON_PIN, gpio_function)
        return pin_class(
            UART_HANDSHAKE_SKIP_BUTTON_PIN,
            pin_class.IN,
            pin_class.PULL_UP,
...
def _skip_button_pressed(button):
    """按钮为指定有效电平时返回 True。"""
    ...
    try:
        return int(button.value()) == int(UART_HANDSHAKE_SKIP_BUTTON_ACTIVE_VALUE)
    except Exception:
        return False
...
class TrackingUART(UARTIO):
    ...
    def wait_for_handshake(
        ...
        skip_button = _make_skip_button()
        while not self.update_handshake():
            if _skip_button_pressed(skip_button):
                _sleep_ms(UART_HANDSHAKE_SKIP_BUTTON_DEBOUNCE_MS)
                if _skip_button_pressed(skip_button):
                    self._peer_ready_received = True
                    self._ready_ack_received = True
                    print("检测到 GPIO{} 按钮，跳过串口握手等待".format(
                        UART_HANDSHAKE_SKIP_BUTTON_PIN,
                    ))
                    return self
