...
Expected: `4 passed` in the focused file and `18 passed` in the full suite.
...
Expected: syntax exits `0`, all `18 passed`, and `git diff --check` prints no errors.
...
Expected: syntax exits `0`, all `18 passed`, the worktree is clean, and the three implementation commits appear after the two design commits.
