...
class WifiRtspService:
    ...
    def _get_wbc(self):
        ...
        try:
            ...
            from tools.safe_wbc_rtsp import SafeWbcRtsp
        except ImportError:
...
def load_wifi_credentials(module_name="wifi_secrets"):
    ...
    if module_name == "wifi_secrets":
        module_names = ("tools.wifi_secrets", "wifi_secrets")

    module = None
    for candidate in module_names:
        try:
            module = __import__(candidate, None, None, ("WIFI_SSID",))
            break
        except ImportError:
            pass
    if module is None:
        raise RuntimeError(
            "missing wifi_secrets.py; copy tools/wifi_secrets.example.py "
            "and set credentials"
        )
