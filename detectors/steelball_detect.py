...
def _is_detection_dict(item):
    """判断字典是否像一个检测结果。"""

    try:
        if "bbox" in item or "box" in item or "rect" in item:
            return True
        if (
...
def count_detections(dets):
    ...
    try:
        for row in dets:
            if _is_detection_row(row) or _is_detection_dict(row):
                count += 1
        if count == 0:
            return int(dets_len)
        return count
...
def run_steelball_detect():
    ...
    try:
        ...
        while True:
            ...
            draw_result = yolo.draw_result(dets, pl.osd_img)
            count = count_detections(dets)
            if count == 0 and draw_result is not None:
                count = count_detections(draw_result)
