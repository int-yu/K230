...
1. 复用 detectors.tangle.RectangleDetector 检测黑框白心方框。
2. 复用 detectors.corner_cycle.order_corners_clockwise 统一四角顺序。
...
- SQUARE_DISTANCE_OBJECT_WIDTH_CM / HEIGHT_CM 是否等于实物尺寸；
- SQUARE_DISTANCE_CAMERA_MATRIX 是否匹配当前摄像头、镜头和采集分辨率；
...
from config import (
    ...
    SQUARE_DISTANCE_DEMO_GC_INTERVAL,
    SQUARE_DISTANCE_DEMO_PRINT_INTERVAL,
    SQUARE_DISTANCE_DIST_COEFFS,
    SQUARE_DISTANCE_DRAW_ERROR_COLOR,
    SQUARE_DISTANCE_DRAW_POINT_COLOR,
    SQUARE_DISTANCE_DRAW_POINT_RADIUS,
    SQUARE_DISTANCE_DRAW_RECTANGLE_COLOR,
    SQUARE_DISTANCE_DRAW_ROI,
    SQUARE_DISTANCE_DRAW_ROI_COLOR,
    SQUARE_DISTANCE_DRAW_TEXT_COLOR,
    SQUARE_DISTANCE_DRAW_THICKNESS,
    SQUARE_DISTANCE_MIN_DISTANCE_CM,
    SQUARE_DISTANCE_OBJECT_HEIGHT_CM,
    SQUARE_DISTANCE_OBJECT_WIDTH_CM,
    SQUARE_DISTANCE_ROI,
)
from core.draw_utils import draw_cv_fps, draw_cv_text
from detectors.corner_cycle import order_corners_clockwise
from detectors.tangle import RectangleDetector, draw_frame_outline
...
def estimate_rectangle_distance(
    points,
    camera_matrix=SQUARE_DISTANCE_CAMERA_MATRIX,
    dist_coeffs=SQUARE_DISTANCE_DIST_COEFFS,
    object_width_cm=SQUARE_DISTANCE_OBJECT_WIDTH_CM,
    object_height_cm=SQUARE_DISTANCE_OBJECT_HEIGHT_CM,
):
    """根据四个角点估计方框中心距离。
...
class SquareDistanceDetector:
    """检测方框并估算相机到方框中心的距离。"""

    def __init__(
        self,
        rectangle_detector=None,
        roi=SQUARE_DISTANCE_ROI,
        object_width_cm=SQUARE_DISTANCE_OBJECT_WIDTH_CM,
        object_height_cm=SQUARE_DISTANCE_OBJECT_HEIGHT_CM,
        camera_matrix=SQUARE_DISTANCE_CAMERA_MATRIX,
        dist_coeffs=SQUARE_DISTANCE_DIST_COEFFS,
        min_distance_cm=SQUARE_DISTANCE_MIN_DISTANCE_CM,
    ):
        if len(roi) != 4:
            raise ValueError("SQUARE_DISTANCE_ROI 必须包含 4 个比例值")
    ...
    def draw(self, frame, result):
        ...
        if result is None:
            ...
            draw_cv_text(
                ...
                5,
                53,
                color=SQUARE_DISTANCE_DRAW_ERROR_COLOR,
            )
            return None
        ...
        for point in result["points"]:
            px = int(point[0])
            py = int(point[1])
            radius = int(SQUARE_DISTANCE_DRAW_POINT_RADIUS)
            cv2.line(
                frame,
                (px - radius, py),
                (px + radius, py),
        ...
        draw_cv_text(
            ...
            5,
            53,
            color=SQUARE_DISTANCE_DRAW_TEXT_COLOR,
        )
        draw_cv_text(
            frame,
...
def run_square_distance_demo(display_target=None):
    """运行摄像头、显示和方框距离检测示例。"""
    ...
    camera = None
    detector = SquareDistanceDetector()
    ...
    try:
        ...
        while True:
            ...
            frame_count += 1
            if frame_count % SQUARE_DISTANCE_DEMO_PRINT_INTERVAL == 0:
                if result is None:
            ...
            if frame_count % SQUARE_DISTANCE_DEMO_GC_INTERVAL == 0:
                gc.collect()
    except Exception:
        print("程序发生错误")
        import sys as _sys
